...
class CSchedule:
    def __init__(self, arriratio):
        ...
        self.deadlinexpo = 0.01  # 0.083 #0.083  ###end time slot according with exppenntioonal  distributioon，0.083=1/1hour=1/12slots
        self.throughputexpo = 1.0 / 20000  # 1.0/20000000#  600000##20Gps, throughput according with exppenntioonal  distributioon
    ...
    def SRCSemiFlexiable(self):
        ...
        while tcur < self.SLOTNUM:
            ...
            while m_reqcount < self.reqperSlot[tcur]:
                ...
                sinkNum = random.randint(int(self.NODENUM * 0.4), int(self.NODENUM * 0.5))
                ...
                while len(new_sink) > 0:
                            ...
                            for t in range(rSlotNum - lastMinTimeCost):
                                ...
                                for p in range(rPathNum):
                                    ...
                                    for linkindex in range(len(self.PathList[src][sink][p])):
                                        ...
                                        linkCostSetontcur.append(LinkResCaperSlot[linkid][t + r_start + minTimeCost])
                                ...
                                # print(tempSize_2)
                                if tempSize_2 >= rSize and t <= rSlotNum - lastMinTimeCost - 1:
                                    numoftimeSlotfromSRCtoDST[s][d] = minTimeCost + t + 1
                                    break

                                elif tempSize_2 < rSize and t == rSlotNum - lastMinTimeCost - 1:
                        ...
                        for t in range(minTimeCost - lastMinTimeCost):
                            ...
                            if (minTimeCost - lastMinTimeCost) == 1:
                                ...
                                for p in range(rPathNum):
                                    ...
                                    partCostperPath = int(rSize * ratio)
                                ...
                                if t < (minTimeCost - lastMinTimeCost - 1):
                                    # print(minTimeCost, lastMinTimeCost, t)
                                    for p in range(rPathNum):
                                        for linkindex in range(len(self.PathList[srctoSinkReadytoMove][sinkReadyMovetosrcList][p])):
                                            linkid = self.PathList[srctoSinkReadytoMove][sinkReadyMovetosrcList][p][linkindex]
                                        ...
                                        freq_2.writelines("%d %d %d %d %d %d" % (t, p, totalSize_2, rSize, self.reqNUM, len(new_sink)))
                                        freq_2.writelines("\n")
                                else:
                                    remainLinkCap = rSize - totalSize_2
                                    if totalSize_2 == 0:
                                        for p in range(rPathNum):
                                            ratio = pathCaptcur[m_min][n_min][t][p] / totalSize
                                            partCostperPath = int(rSize * ratio)
                                        ...
                                        ratio = remainLinkCap / totalSize_2
                                        for p in range(rPathNum):
                                            costperLink = pathCaptcur[m_min][n_min][t][p] * ratio
                    ...
                    if minTimeCost >= rSlotNum:
                        ...
                        # print(lastMinTimeCost)
                        judgeVariate = True

                if judgeVariate == True:
                    self.acceptReqs += 1
