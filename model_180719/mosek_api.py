...
def maxThroughputmodel(pathnum, linknum, destnum, fsize, slotnum, linkonpath, linktimecap, remainratio,
                       assignedsrcfordest, sourcelist):
    srcnum = destnum + 1
    print "\n solve problem: pathnum, linkum, destnum, srcnum, slotnum ", pathnum, linknum, destnum, srcnum, slotnum

    with Model("mipmodel") as M:
        x = M.variable('X', [srcnum, destnum, slotnum],
                       Domain.greaterThan(0.0))  # the data amount transferred from src to dest in every timeslot
        # y = M.variable('Y', [srcnum, destnum, pathnum, slotnum], Domain.greaterThan(0.0))
        y = M.variable('Y', [srcnum * destnum, pathnum, slotnum],
                       Domain.greaterThan(0.0))  # resource allocation on path in every timeslot
        ...
        w = M.variable('w', destnum,
                       Domain.greaterThan(0.))  # the data amount received at a dest before the end of the timeline used
